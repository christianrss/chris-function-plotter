...
constexpr auto WINDOW_WIDTH = std::uint32_t{640};
...
void end_cycle(GLFWwindow *const window)
{
    const auto clear_color =
        ImVec4(30.0F / 255.0F, 3.0F / 255.0F, 30.0F / 255.0F, 1.00F);
        int display_w = 0;
        int display_h = 0;
...
int main(int, char **)
{
...
#if defined(IMGUI_IPL_OPENGL_ES2)
    // GL ES 2.0 + GLSL 100
    ...
    style.Colors[ImGuiCol_TableBorderStrong] = ImVec4(1.0F, 1.0F, 1.0F, 1.0F);
